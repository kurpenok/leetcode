...
pub fn zigzag_level_order(root: Option<Rc<RefCell<TreeNode>>>) -> Vec<Vec<i32>> {
    ...
    for (i, key) in depth_keys.iter().enumerate() {
        let mut row: Vec<i32> = values_by_depth[&key].clone();
        if i % 2 == 1 {
            row.reverse();
        }
        result.push(row);
...
mod test {
    ...
    fn test_case_1() {
        assert_eq!(
            zigzag_level_order(Some(Rc::new(RefCell::new(TreeNode {
                val: 3,
    ...
    fn test_case_2() {
        assert_eq!(
            zigzag_level_order(Some(Rc::new(RefCell::new(TreeNode::new(1))))),
            vec![vec![1]]
        );
    }
    ...
    fn test_case_3() {
        ...
        assert_eq!(zigzag_level_order(None), expected_result);
    }
}
